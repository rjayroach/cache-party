  ...
  module Facebook
    describe 'Helpers' do
      ...
      describe "#url_for", job: true do
        ...
        it 'returns the cover source of a Facebook Page' do
          subject = build(:cache_party_facebook_page, :joltcafe)
          VCR.use_cassette("facebook/joltcafe") do
            expect(File.basename(URI.parse(subject.url_for(:object) {|data| data['cover']['source']}).path)).to eq(
              '562219_378872905484694_26367699_n.jpg'
            )
          end
        end
        ...
        it 'returns the picture of a Facebook User' do
          ...
          VCR.use_cassette("facebook/davetone_basic") do
            expect(File.basename(URI.parse(subject.url_for(:picture)).path)).to eq '174153_100005787840155_2101067759_q.jpg'
          end
        end
